...
def generate_demo_data(n=180, seed=42) -> pd.DataFrame:
    ...
    def _fit_probs(codes, probs):
        probs = list(probs)[: len(codes)]
        if len(probs) < len(codes):
            probs += [probs[-1] if probs else 1.0] * (len(codes) - len(probs))
        probs = np.array(probs, dtype=float)
        return probs / probs.sum()

    vinculo_codes = list(FIELD_META["vinculo_institucional"]["choices"].keys())
    vinculo_p = _fit_probs(vinculo_codes, [0.28, 0.24, 0.10, 0.06, 0.08, 0.05, 0.06, 0.05, 0.08])
    categoria_codes = list(FIELD_META["categoria_profissional"]["choices"].keys())
    categoria_p = _fit_probs(categoria_codes, [0.16, 0.14, 0.16, 0.14, 0.18, 0.12, 0.06, 0.04])
    ...
    eventos_nomes = [
        "Congresso Brasileiro de Clínica Médica", "Jornada Paulista de Pediatria",
        "Congresso Internacional de Oncologia", "Semana Científica FAMERP",
        "Congresso Brasileiro de Cirurgia", "Simpósio de Enfermagem FUNFARME",
    ]
    comentarios_pool = [
        "Poderia haver mais divulgação sobre os prazos de submissão de projetos.",
        "O CENAP poderia oferecer oficinas de escrita científica com mais frequência.",
        "Falta tempo na rotina assistencial para me dedicar à pesquisa.",
        "Gostaria de apoio para análise estatística dos dados coletados.",
        "O apoio recebido do CENAP na última submissão foi excelente.",
        "", "", "",
    ]
    ...
    for i in range(n):
        ...
        row["vinculo_institucional"] = vinculo
        row["categoria_profissional"] = categoria
        row["setor_trabalho"] = rng.choice(setores)
        row["participa_pesquisa"] = participa
        row["publicacao_cinco_anos"] = publicou
        row["eventos_cientificos"] = eventos
        row["apresentacao_trabalho_evento"] = apresentou
        row["evento_apresentado"] = rng.choice(eventos_nomes) if apresentou == "1" else ""
        row["importancia_pesquisa_inovacao"] = likert(bias=0.10)
        row["pesquisa_desenvolvimento_profissional"] = likert(bias=0.0 if participa == "1" else -0.10)
        row["conhecimento_cenap"] = conhece_cenap
        row["utilizou_cenap"] = utilizou
        row["atendimento_cenap"] = likert(bias=0.05) if utilizou == "1" else likert(bias=-0.15)
        row["orientacoes_cenap"] = likert(bias=0.05) if utilizou == "1" else likert(bias=-0.15)
        row["servicos_cenap"] = likert(bias=0.05) if utilizou == "1" else likert(bias=-0.15)
        row["papel_cenap_instituicao"] = likert(bias=0.10)
        row["comentarios_sugestoes"] = rng.choice(comentarios_pool)

        obst_choice = rng.choice(obst_opts, size=min(int(rng.integers(1, 4)), len(obst_opts)), replace=False)
        for c in obst_opts:
...
def get_selected_label(key: str):
    ...
    if not ev:
        return None
    points = ev.get("selection", {}).get("points", [])
    if not points:
        return None
    return points[0].get("y")
...
with tab_perfil:
    ...
    with c1:
        vinc = decode_series(base_filtered["vinculo_institucional"], "vinculo_institucional").value_counts()
        fig = px.bar(
        ...
        st.plotly_chart(fig, use_container_width=True, on_select="rerun", key="chart_vinculo")
        st.caption("💡 Clique em uma barra para filtrar todo o dashboard por essa unidade.")
...
with tab_cenap:
    ...
    st.markdown("#### Já utilizou os serviços do CENAP?")
    util_counts = decode_series(base_filtered["utilizou_cenap"], "utilizou_cenap").value_counts()
    fig_util = px.bar(
        util_counts.sort_values(ascending=True), orientation="h",
        color_discrete_sequence=[SECONDARY],
        labels={"value": "Respondentes", "index": ""},
    )
    fig_util.update_layout(showlegend=False, height=220, margin=dict(l=10, r=10, t=10, b=10))
    st.plotly_chart(fig_util, use_container_width=True, on_select="rerun", key="chart_cenap_uso")
    st.caption("💡 Clique em 'Sim' ou 'Não' para filtrar o dashboard pelos respondentes que já utilizaram (ou não) o CENAP.")
